  ...
  function initFAQ(options) {
    ...
    accordions.forEach(function (accordion) {
      ...
      if (icon) {
        tl.to(
          icon,
          { rotation: opts.iconRotation, duration: opts.duration, ease: opts.ease },
          0
        );
      }
    ...
    // Si un accordéon est ouvert par défaut (classe is-open déjà posée),
    // on fige sa hauteur à auto au load pour éviter un saut visuel.
    window.addEventListener("load", function () {
      accordions.forEach(function (accordion) {
        if (accordion.classList.contains(opts.openClass)) {
          var answer = accordion.querySelector(opts.answerSelector);
          if (answer) gsap.set(answer, { height: "auto" });
          if (accordion._baryonTl) accordion._baryonTl.progress(1).pause();
        }
      });
    });
